...
public class move : MonoBehaviour
{
    ...
    void Rotation()
    {
        ...
        float dx = this.transform.position.x - target.position.x;
        float dy = this.transform.position.y - target.position.y;

        float angle = Mathf.Atan2(dy, dx) * Mathf.Rad2Deg;

        Quaternion rot = Quaternion.Euler(new Vector3(0, 0, angle + 270));
